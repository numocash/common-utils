...
export const calculateVirtualPrice = (exchange: IExchangeInfo): Fraction | null => {
  ...
  const price = new Fraction(
    computeD(exchange.ampFactor, exchange.reserves[0].raw, exchange.reserves[1].raw),
    amount.raw
  )
  return price
}
...
export const calculateEstimatedSwapOutputAmount = (
  ...
  const fee = new TokenAmount(toReserves.token, exchange.fees.trade.asFraction.multiply(amountBeforeFees).toFixed(0))

  const adminFee = new TokenAmount(toReserves.token, exchange.fees.admin.asFraction.multiply(fee.raw).toFixed(0))
...
export const calculateEstimatedWithdrawAmount = ({
  ...
  }) as [[TokenAmount, { beforeFees: JSBI; fee: JSBI }], [TokenAmount, { beforeFees: JSBI; fee: JSBI }]]
  ...
  return {
    ...
    fees: withdrawAmounts.map(([amt, { fee }]) => new TokenAmount(amt.token, fee)) as [TokenAmount, TokenAmount],
  }
}
